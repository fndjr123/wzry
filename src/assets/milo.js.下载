...
extend(loginStatus,{
	...
	syncToQQ:function(domain,oCookie,callback){
		...
		this.init(service,location.protocol+'//'+domain+'/comm-htdocs/login/asyncLoginStatus.html',function(){
			...
			for(var k in oCookie){
				...
				newCookie.push(([encodeURIComponent(k),encodeURIComponent(oCookie[k]),encodeURIComponent('/'),600]).join('='));
			}
			if('function' == typeof window.postMessage){
				...
				iframe.contentWindow.postMessage('loginStatus|'+newCookie.join(',')+'|'+callbackName,'*');
			}
		})
	},
	/**
	...
	syncToAME:function(callback){
		...
		self.clearAME(function(){
			...
			*/
			if("function" != typeof window.postMessage){//不支持postMessage
				var newCookie={};
				for(var k in oCookie){
					newCookie[k]=[oCookie[k],'/',600];
				}
